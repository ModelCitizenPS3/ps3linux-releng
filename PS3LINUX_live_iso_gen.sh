...
chroot $CHROOT_PATH /usr/bin/dnf -y --releasever=28 --forcearch=ppc64 --setopt=install_weak_deps=False --setopt=tsflags=nodocs install udisks2-zram nfs-utils bash-completion wget tar wpa_supplicant dosfstools busybox
...
if [ -d "$INITRAMFS_PATH" ]; then
    ...
    mkdir -p $INITRAMFS_PATH/{dev,lib/modules,mnt/{iso,lower,sysroot,upper},proc,run,sys,tmp,usr/{bin,sbin}}
