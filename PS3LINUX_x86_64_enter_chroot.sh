...
/bin/bash -c "chroot $CHROOT_PATH /usr/bin/env -i ARCH=powerpc CROSS_COMPILE=powerpc64-linux-gnu- HOME=/root TERM=$TERM PS1='\u:\w\$ ' PATH=/root/.local/bin:/usr/lib64/ccache:/usr/bin:/usr/sbin:/bin:/sbin /bin/bash --login"
